...
namespace SeatManageWebQUI.WebService
{
    /// <summary>
    ...
    public class SeatLayoutInterface : System.Web.Services.WebService
    {
        ...
        {
            if (!IsCanBespeak(schoolNo, roomNo, Date))
            {
                return "你的操作不合法，请使用正确途径预约座位";
            }
            else
            {
                string json = drowSeatLayoutHtml(schoolNo, studentNo, roomNo, Date, Context.Request.Params["divTransparentTop"], Context.Request.Params["divTransparentLeft"]);
                return json;
            }
        }

        private string drowSeatLayoutHtml(string schoolNo, string studentNo, string roomNo, string Date, string divTransparentTop, string divTransparentLeft)
        {
            DateTime date;
            try
            {
            ...
            try
            {
                //string sn = Context.Request.Params[ConfigurationManager.AppSettings["SchoolNo"]].ToString();
                if (schoolNo != "20180716")
                {
                    return "请确认学校编号";
                }
            }
            ...
            try
            {
                SeatManage.ClassModel.SeatLayout _SeatLayout = SeatManage.Bll.T_SM_SeatBespeak.GetBeseakSeatLayout(roomNo, date);
                if (_SeatLayout == null)
                {
                    return "获取阅览室座位布局出错了";
                }
                ...
                foreach (SeatManage.ClassModel.Seat seat in _SeatLayout.Seats.Values)
                {
                    ...
                    {
                        tipContent = "可预约";
                        urlParameters = string.Format("seatNo={0}&seatShortNo={1}&date={2}&roomNo={3}", seat.SeatNo, seat.ShortSeatNo, date.ToBinary(), roomNo);
                    }
